...
bool EPOS::setAllDigitalOutputs(bool state){
        int loopCounter=0;
        while(!VCS_SetAllDigitalOutputs(keyHandle, nodeID, int(state), &errorCode)){
                if(loopCounter++>maxCommandAttempts){
                        ROS_ERROR_STREAM("Could not set all digital outputs on EPOS: " << getErrorInfo(errorCode));
                        return false;
                }
        }
        return true;
...
bool EPOS::getAllDigitalInputs(unsigned short* inputs){
        int loopCounter=0;
        while(!VCS_GetAllDigitalInputs(keyHandle, nodeID, inputs, &errorCode)){
                if(loopCounter++>maxCommandAttempts){
                        ROS_ERROR_STREAM("Could not get digital inputs on EPOS: " << getErrorInfo(errorCode));
                        return false;
                }
        }
        return true;
